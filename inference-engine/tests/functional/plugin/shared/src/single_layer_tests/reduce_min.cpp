...
std::string ReduceMinLayerTest::getTestCaseName(testing::TestParamInfo<reduceMinParams> obj) {
...
void ReduceMinLayerTest::SetUp() {
    ...
    const auto AxesOp = std::make_shared<ngraph::opset1::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{axes.size()},
                                                                  axes);
    const auto reduce_min = std::make_shared<ngraph::opset1::ReduceMin>(paramOuts.at(0), AxesOp);

    ngraph::ResultVector results;
    results.push_back(std::make_shared<ngraph::opset1::Result>(reduce_min));
    fnPtr = std::make_shared<ngraph::Function>(results, params, "reduce_min");
}

TEST_P(ReduceMinLayerTest, CompareWithRefs) {
    inferAndValidate();
};
