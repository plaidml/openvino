...
std::string ReduceSumLayerTest::getTestCaseName(testing::TestParamInfo<reduceSumParams> obj) {
...
void ReduceSumLayerTest::SetUp() {
    ...
    const auto AxesOp = std::make_shared<ngraph::opset1::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{axes.size()},
                                                                  axes);
    const auto reduce_sum = std::make_shared<ngraph::opset1::ReduceSum>(paramOuts.at(0), AxesOp);

    ngraph::ResultVector results;
    results.push_back(std::make_shared<ngraph::opset1::Result>(reduce_sum));
    fnPtr = std::make_shared<ngraph::Function>(results, params, "reduce_sum");
}

TEST_P(ReduceSumLayerTest, CompareWithRefs) {
    inferAndValidate();
};
