...
std::string ReduceProdLayerTest::getTestCaseName(testing::TestParamInfo<reduceProdParams> obj) {
...
void ReduceProdLayerTest::SetUp() {
    std::vector<int64_t> axes;
    bool keep_dims;
    ...
    const auto AxesOp = std::make_shared<ngraph::opset1::Constant>(ngraph::element::Type_t::i64, ngraph::Shape{axes.size()},
                                                                  axes);
    const auto reduce_prod = std::make_shared<ngraph::opset1::ReduceProd>(paramOuts.at(0), AxesOp);

    ngraph::ResultVector results;
    results.push_back(std::make_shared<ngraph::opset1::Result>(reduce_prod));
    fnPtr = std::make_shared<ngraph::Function>(results, params, "reduce_prod");
}

TEST_P(ReduceProdLayerTest, CompareWithRefs) {
    inferAndValidate();
};
