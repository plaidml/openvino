...
std::string ExtractImagePatchesTest::getTestCaseName(const testing::TestParamInfo<extractImagePatchesTuple> &obj) {
    ...
    std::tie(inputShape, kernel, strides, rates, padType, netPrc, inPrc, outPrc, inLayout, targetName) = obj.param;
...
void ExtractImagePatchesTest::SetUp() {
    ...
    std::tie(inputShape, kernel, strides, rates, padType, netPrecision, inPrc, outPrc, inLayout, targetDevice) = this->GetParam();
    ...
    auto extImgPatches = std::make_shared<ngraph::opset3::ExtractImagePatches>(
        inputNode, ngraph::Shape(kernel), ngraph::Strides(strides), ngraph::Shape(rates), padType);
